...
/**
 * Тестовые данные для всех инфоблоков главной страницы.
 *
 * ICON_CODE — строковый код иконки; SVG рендерится в Blade через @include('icons._code_').
 * Доступные коды: thumbs-up | truck | dollar | wrench | remote | energy | star | shield
 */
class Version20260514000006 extends Version
{
    ...
    private function seedGreeCards(HelperManager $helper): void
    {
        ...
        foreach ($cards as $c) {
            ...
            $this->out('Карточка Gree: %s [%d]', $c['fields']['NAME'], $elemId);
        }
    ...
    private function seedTechnologies(HelperManager $helper): void
    {
        ...
        $items = [
            ...
            ['NAME' => 'Система самоочистки',              'CODE' => 'tech-selfclean',    'PREVIEW_TEXT' => 'Автоматическая промозарка и сушка испарителя каждые 8 часов работы.'],
