...
/**
 * Reads catalog-page "Why Gree" cards + stats from their dedicated iblocks.
 * Same shape as Home/Brand repos so the three pages can be edited independently.
 */
final class CatalogRepository extends BaseRepository implements CatalogRepositoryInterface
{
    public function getGreeCards(): GreeCardCollection
    {
        \Bitrix\Main\Loader::includeModule('iblock');

        $iblockId = $this->resolveIblockId(IblockCode::CatalogGreeCards);
        if (!$iblockId) {
    ...
    public function getGreeStats(): GreeStatCollection
    {
        ...
        $iblockId = $this->resolveIblockId(IblockCode::CatalogGreeStats);
        if (!$iblockId) {
